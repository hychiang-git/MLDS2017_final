...
class BNLSTMCell(RNNCell):
    ...
    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            return new_h, (new_c, new_h)#, (xh, hh, bn_xh, bn_hh, bn_new_c)
...
def batch_norm(x, name_scope, training, epsilon=1e-3, decay=0.999):
    ...
    with tf.variable_scope(name_scope):
        ...
        b_mean = tf.get_variable('b_mean', [size], initializer=tf.zeros_initializer, trainable=False)
        b_var = tf.get_variable('b_var', [size], initializer=tf.ones_initializer, trainable=False)
        batch_mean, batch_var = tf.nn.moments(x, [0])

        train_mean_op = tf.assign(pop_mean, pop_mean * decay + batch_mean * (1 - decay))
        train_var_op = tf.assign(pop_var, pop_var * decay + batch_var * (1 - decay))

        test_mean_op = tf.assign(b_mean,batch_mean)
        test_var_op = tf.assign(b_var,batch_var)
        ...
        def population_statistics():
            with tf.control_dependencies([test_mean_op,test_var_op]):
                bn = tf.nn.batch_normalization(x, pop_mean, pop_var, offset, scale, epsilon)
                bn_mean = tf.get_variable('bn_mean', [size], initializer=tf.zeros_initializer, trainable=False)
                bn_var = tf.get_variable('bn_var', [size], initializer=tf.ones_initializer, trainable=False)
                mean, var = tf.nn.moments(bn, [0])
                bn_mean_op = tf.assign(bn_mean,mean)
                bn_var_op = tf.assign(bn_var,var)
                with tf.control_dependencies([bn_mean_op,bn_var_op]):
                    return bn
